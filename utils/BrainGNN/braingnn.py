...
class Network(torch.nn.Module):
    def __init__(self, indim, ratio, nclass, k=8, R=200):
        ...
        self.fc3 = torch.nn.Linear(self.dim3, 1)
    ...
    def forward(self, data):
        ...
        x, edge_index, edge_attr, batch, perm, score1 = self.pool1.forward(x, edge_index, edge_attr, batch)
        ...
        x, edge_index, edge_attr, batch, perm, score2 = self.pool2.forward(x, edge_index,edge_attr, batch)
        ...
        x= F.dropout(x, p=0.5, training=self.training)
        logit = self.fc3(x)  # (B,1) raw logits
        
        return logit.view(-1), 0  # (B,)
