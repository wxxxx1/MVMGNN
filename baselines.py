...
from utils.mvgnn import GCN, GCN_view, GCN_aview, SGCN_view
...
class GCN_Trainer:
    ...
    def __init_model__(self):
        ...
        if self.params.model == 'GCN_view':
            self.model = (GCN_view(in_dim=self.params.feature_channel,
        ...
        elif self.params.model == 'GCN_aview':
            self.model = (GCN_aview(in_dim=self.params.feature_channel,
                                    in_dim2=self.params.feature_channel2,
                                    node_num=self.params.feature_num,
                                    hidden_dim=self.params.gnn_feat,
                                    num_layers=self.params.gnn_layers,
                                    dropout=self.params.dropout_p, mlp_hidden=self.params.gnn_feat).to(self.device))
        elif self.params.model == 'SGCN_view':
            self.model = (SGCN_view(in_dim=self.params.feature_channel,
                                    in_dim2=self.params.feature_channel2,
                                    node_num=self.params.feature_num,
                                    hidden_dim=self.params.gnn_feat,
                                    num_layers=self.params.gnn_layers,
                                    dropout=self.params.dropout_p, mlp_hidden=self.params.gnn_feat).to(self.device))
        elif self.params.model == 'GCN1':
            self.model = (GCN(in_dim=self.params.feature_channel,
                              in_dim2=self.params.feature_channel2,
                              node_num=self.params.feature_num,
                              hidden_dim=self.params.gnn_feat,
                              num_layers=self.params.gnn_layers,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default="GCN_sview", choices=["GCN_view", "GCN_sview",
                                                                           "SGCN", "SGCN_view"])
    ...
    for ii in ["1", "2", "3", "4", "5"]:
        for i in ["GCN1"]:
            for j in ["AD-CN", "AD-MCI", "CN-MCI"]:
                print(f"------------------------   Start_{ii}_{j}   --------------------------------")
                args.exp = os.path.join("base", i, ii, j)
                args.model = i
